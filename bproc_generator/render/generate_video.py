...
def get_day_timesteps(date: str, latitude: float, longitude: float,
    ...
    """
    Returns all renderable UTC timesteps for a given date, bounded by civil
    twilight (solar zenith ≤ 96°), at the given interval.
    ...
    def _round_up(dt: pd.Timestamp) -> pd.Timestamp:
        m     = interval_minutes
        extra = dt.minute % m
        if extra == 0 and dt.second == 0:
            return dt.replace(second=0, microsecond=0)
        return (dt + pd.Timedelta(minutes=m - extra)).replace(
...
class BlenderProcRenderer(RenderParams):
    ...
    def save_video_frame(self, frame_idx: int, video_dir: Path,
        ...
        hdf5_file = hdf5_path / f"{pose_i}.hdf5"
