...
- Per-timestep     : colormap stretches to the day's min/max 
- Diffuse shading  : face color multiplied by Lambert factor from a fixed
                     overhead-south light → adds 3D depth without outlines

Output structure
----------------
Current mode — video frames (active):
  output_path/
    incident/        ← E_i [W/m²] — incident irradiance, one PNG per timestep
      0000.png, 0001.png, ...
    absorbed/        ← q_i = α_i · E_i [W/m²] — absorbed heat flux, one PNG per timestep
      0000.png, 0001.png, ...
    summary.json     ← per-frame solar metadata + ffmpeg command to assemble video

Alternative mode — per-timestep subfolders (commented out, see _save_flux_outputs):
  output_path/
    10h00/
      incident/
        scalars.npy      ← raw E_i values per face
        vectors.npy      ← E_i · n̂_i per face
        heat_flux.ply    ← coloured 3D mesh export
        0000_flux.png …  ← PNG render per camera pose
      absorbed/
        (same structure)
      meta_data.json
    10h30/
...
convention as generate.py so shadow geometry is always coherent.
"""
...
def _find_nearest_meta(output_path: Path, hhmm: str) -> Path | None:
    ...
    generate.py may have run at a coarser interval (e.g. 30 min) while
    ...
    Parameters
    ----------
    output_path : Path  — root output folder (same as generate.py)
...
class HeatFluxParams:
    ...
    """
    Root folder containing the existing generate.py outputs
...
class HeatFluxRunner(HeatFluxParams):
    ...
    def run(self):
        ...
        print(f"  Colormap  : Turbo, daily scale")
        print(f"{'═'*55}\n")
